...
export default function Editor() {
  ...
  const handleCloneCard = (idx) => {
    if (!currentData) return;
    const newData = { ...currentData };
    const cardToClone = JSON.parse(JSON.stringify(newData.cards[idx]));
    cardToClone.id = `card_${Date.now()}_${Math.floor(Math.random() * 1000)}`;
    
    newData.cards.splice(idx + 1, 0, cardToClone);
    
    setCurrentData(newData);
    setSelectedCardIdx(idx + 1);
    setSelectedElementIdx(null);
    
    const previewData = { ...newData, cards: [cardToClone] };
  ...
  const handleMoveCard = (idx, direction) => {
    ...
    if (targetIdx < 0 || targetIdx >= currentData.cards.length) return;
    
    const newData = { ...currentData };
    const temp = newData.cards[idx];
    newData.cards[idx] = newData.cards[targetIdx];
    newData.cards[targetIdx] = temp;
    
    setCurrentData(newData);
    setSelectedCardIdx(targetIdx);
    setSelectedElementIdx(null);
    
    const previewData = { ...newData, cards: [newData.cards[targetIdx]] };
    updatePreview(previewData);
  };
  ...
  return (
    <div className="app-container" onMouseUp={handleDragEnd} style={{ width: '100%', height: '100%' }}>
